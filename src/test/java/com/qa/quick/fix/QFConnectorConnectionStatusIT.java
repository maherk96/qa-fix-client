...
/**
 * Integration tests for QFConnector connection status reporting. Tests various connection state
 * checking methods.
 */
...
class QFConnectorConnectionStatusIT {
  ...
  private QFConnector createTradeOnlyConnector() {
    ...
    return new QFConnector(CLIENT_NAME, commonSettings, clientDef, connEnv, null, null, null, null);
  }
  ...
  void testIsConnected_BothSessions_OnlyOneConnected_ReturnsFalse() throws Exception {
    ...
    assertThat(connector.hasQuoteSession()).as("Should have quote session capability").isTrue();
  }
  ...
  void testGetStatus_ReturnsAccurateSnapshot() throws Exception {
    ...
    assertThat(status.tradeSessionId()).as("Trade session ID should be populated").isNotNull();
    assertThat(status.tradeSessionId().getSenderCompID()).isEqualTo(CLIENT_NAME + "_TRADE");
    assertThat(status.tradeSessionId().getTargetCompID()).isEqualTo("TRADE_SERVER");

    assertThat(status.quoteSessionId()).as("Quote session ID should be populated").isNotNull();
    assertThat(status.quoteSessionId().getSenderCompID()).isEqualTo(CLIENT_NAME + "_QUOTE");
    assertThat(status.quoteSessionId().getTargetCompID()).isEqualTo("QUOTE_SERVER");
    ...
    assertThat(status.tradeSessionId()).as("Trade session ID should be null after stop").isNull();
    assertThat(status.quoteSessionId()).as("Quote session ID should be null after stop").isNull();
  }
}
