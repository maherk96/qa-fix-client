...
/**
 * Integration tests for QFConnector basic lifecycle operations. Tests start, stop, restart, and
...
class QFConnectorLifecycleIT {
  ...
  void testStart_FreshConnector_ConnectsSuccessfully() throws Exception {
    ...
    assertThat(connected).as("Connector should connect within 10 seconds").isTrue();

    assertThat(connector.isConnected()).as("Connector should report as connected").isTrue();
  ...
  void testStart_AlreadyStarted_IsIdempotent() throws Exception {
    ...
    assertThat(connector.isConnected()).as("Connector should still be connected").isTrue();
  ...
  void testStop_WhenStarted_DisconnectsSuccessfully() throws Exception {
    ...
    assertThat(connector.isConnected()).as("Connector should be disconnected after stop").isFalse();
  ...
  void testStop_WhenNotStarted_IsIdempotent() {
    ...
    assertDoesNotThrow(
        () -> connector.stop(), "Stop should not throw when connector was never started");

    // Verify state remains consistent
    assertThat(connector.isConnected()).as("Connector should not be connected").isFalse();
  }
  ...
  void testRestart_ConnectedSession_ReconnectsSuccessfully() throws Exception {
    ...
    assertThat(reconnected).as("Connector should reconnect after restart").isTrue();

    assertThat(connector.isConnected()).as("Connector should be connected after restart").isTrue();
  ...
  void testWaitForConnection_BeforeStart_ReturnsFalse() throws Exception {
    ...
    assertThat(connected).as("WaitForConnection should return false when not started").isFalse();
  ...
  void testWaitForConnection_AfterStart_WaitsForConnection() throws Exception {
    ...
    assertThat(waitStarted.await(2, TimeUnit.SECONDS)).as("Wait thread should start").isTrue();

    // Wait should complete when connection happens
    assertThat(waitCompleted.await(12, TimeUnit.SECONDS)).as("Wait should complete").isTrue();
    ...
    assertThat(connector.isConnected()).as("Connector should be connected").isTrue();
  }
  ...
  void testWaitForConnection_Timeout_ReturnsFalse() throws Exception {
    ...
    connector =
        new QFConnector(
            "TIMEOUT_CLIENT", commonSettings, clientDef, connEnv, null, null, null, null);
    ...
    assertThat(connected).as("WaitForConnection should return false on timeout").isFalse();
